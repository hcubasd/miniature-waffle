...
describe("generateBackgrounds", () => {
	it("reduces 1x1 backgrounds to the nominal effective-circle colors", () => {
		...
		expect(Array.from(backgrounds[0]!.data)).toEqual([251, 157, 186, 255]);
		expect(Array.from(backgrounds[1]!.data)).toEqual([252, 157, 184, 255]);
		expect(new Set(backgrounds.map((background) => background.startColor)).size).toBe(
			256,
		);
	});
	...
	it("spreads extra scanlines across bands when height exceeds the band count", () => {
		...
		expect(background.bands.filter((band) => band.height === 2)).toHaveLength(1);
		expect(background.bands.filter((band) => band.height === 1)).toHaveLength(255);
	});
});
